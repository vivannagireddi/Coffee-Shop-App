...
LoginStatus = {{ isLoggedIn|tojson }};

document.querySelectorAll(".card").forEach((card) => {
  const price = parseFloat(card.dataset.price || "0");
  
  // get the dropdown element (first one if multiple)
  const cupsizeElement = card.querySelector(".myDropdown") || card.querySelector("select");
  ...
  addToCartButton.addEventListener("click", () => {
    if (LoginStatus === "true") {
      ...
      console.log(`Added to cart: Item ${itemName}, Size ${selectedOption}, Quantity: ${quantity}, Price: ₹${itemPrice}`);
      ...
      fetch('/add-to-cart', {
        ...
        body: JSON.stringify({ itemName, quantity, size: selectedOption, price: itemPrice }),
      })
      ...
      deleteItem.addEventListener("click", () => {
        ...
        console.log(`Removed from cart: Item ${itemName}, Size ${selectedOption}, Quantity: ${quantity}`);
        deleteItem.remove();
      });
    } else {
